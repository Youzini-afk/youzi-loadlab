...
def create_secret_endpoint(payload: SecretCreate, session: Session = Depends(get_session)) -> SecretRead:
    init_db()
    return create_secret(session, payload)


@router.get("", response_model=list[SecretRead])
def list_secrets_endpoint(session: Session = Depends(get_session)) -> list[SecretRead]:
    init_db()
