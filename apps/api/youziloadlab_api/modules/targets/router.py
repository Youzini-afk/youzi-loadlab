...
def create_target_endpoint(payload: TargetCreate, session: Session = Depends(get_session)) -> Target:
    init_db()
    return create_target(session, payload)


@router.get("", response_model=list[TargetRead])
def list_targets_endpoint(session: Session = Depends(get_session)) -> list[Target]:
    init_db()
